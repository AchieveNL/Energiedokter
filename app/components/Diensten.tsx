  ...
  const Card = ({
    ...
    <div className="bg-[#F6F6F6] flex flex-col gap-5 items-center justify-between rounded-tl-[3rem] rounded-br-[3rem] px-8 py-4 hover:bg-[#F0F8E2] hover:-translate-y-1 transition-all duration-300 ">
      <div className="flex justify-between w-full gap-3 ">
        <img src={`/assets/icons/service${icon}.svg`} className="size-10" alt="" />
        <h1 className="font-semibold md:max-w-2/3 text-right md:text-[17px] ">{title}</h1>
      </div>
      <p className="text-sm text-[#4D4D4D] ">{text}</p>
      <Link href="#" className="self-start text-[#4B7939] text-sm ">Lees meer</Link>
    </div>
  );
  return (
    <div
      style={{ fontFamily: "Poppins" }}
      className="mb-96 flex flex-col items-center mt-20"
    >
      <SectionTitle
        title="Diensten"
        span={
