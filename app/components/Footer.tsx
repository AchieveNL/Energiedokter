...
export default function Footer() {
  ...
  useEffect(() => {
    const logImageHeight = () => {
      ...
      // Add this to measure the mobile div height
      if (MobiledivRef.current) {
        setDivHeight(MobiledivRef.current.offsetHeight);
      }
    };
    ...
    // Add listener for mobile div
    const mobileDiv = MobiledivRef.current;
    if (mobileDiv) {
      const resizeObserver = new ResizeObserver(logImageHeight);
      resizeObserver.observe(mobileDiv);
    ...
    <>
      <div>
        <img
          ...
          className="w-full lg:mb-0 -mb-px lg:hidden"
          alt=""
        />
        <div className="bg-[#254055] p-px -mb-px lg:hidden "></div>
        <div
          className="mt-0 bg-[#254055] lg:pt-0 relative -z-20 "
          style={{ fontFamily: "Poppins" }}
        >
          {/* mobile light */}
          <img
            src="/assets/footer/mobile-spotlight.svg"
            className="lg:hidden block absolute top-0 right-1/2 translate-x-1/2"
            alt=""
          />
          <img
            src="/assets/footer/mobile-light.svg"
            className="lg:hidden block absolute top-0 right-1/2 translate-x-1/2 flicker-light"
            alt=""
          />
          {/* body */}
          <div
            ref={MobiledivRef}
            className="flex lg:flex-row flex-col text-[#FEEEF3] justify-center lg:items-start items-center md:gap-10 lg:gap-4 xl:gap-12 2xl:gap-15 gap-5 z-10 absolute right-1/2 lg:translate-x-[57%] translate-x-1/2 min-[768px]:mt-64 min-[1024px]:mt-7 min-[1025px]:mt-20 mt-15 min-[1536px]:mt-40 min-[1900px]:mt-44 min-[2300px]:mt-60 min-[2600px]:mt-70 md:w-2/3 scale-[1] min-[768px]:scale-[1.5] min-[1024px]:scale-[1.1] min-[1536px]:scale-[1.3] min-[2300px]:scale-[1.5]"
          >
